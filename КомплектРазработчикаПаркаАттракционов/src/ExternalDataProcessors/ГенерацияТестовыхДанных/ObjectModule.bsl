...
Процедура СгенерироватьДанные() Экспорт
	...
	Пока Курсор < Период.ДатаОкончания Цикл
		...
		Для НомерПосетителя = 1 По КоличествоПосетителей Цикл
			...
			ИндексСлучайногоПосетителей = ГСЧ.СлучайноеЧисло(0, 99);
			
			ФИОПосетителя = СписокПосетителей[ИндексСлучайногоПосетителей];
...
Функция КупитьБилет(Посетитель, Аттракцион, ДатаПосещения, ГСЧ);
	...
	ИндексНоменклаутры = ГСЧ.СлучайноеЧисло(0, ДоступнаяНоменклатура.ВГраница());
	
	Номенклатура = ДоступнаяНоменклатура[ИндексНоменклаутры];
